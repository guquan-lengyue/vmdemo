...
本节为在 `vmdemo` 后端和当前主机上运行 GPU 虚拟化提供建议、命令、以及与本项目后端的集成方式。
...
- 检查 GPU 设备：
  ```bash
  lspci -nnk | egrep -i 'vga|display|3d|intel|nvidia|amd'
  ```
- 检查是否支持 mdev（Intel GVT-g）：
  ```bash
  ls /sys/bus/pci/devices/0000:00:02.0/mdev_supported_types  # 对于 0000:00:02.0 iGPU
  ```

### 2) 推荐技术栈（基于主机与需求）
- Intel iGPU（集成）优先 -> Intel GVT-g（中介虚拟设备，mdev）：支持多个 vGPU，适合需要同时运行宿主/来宾图形的场景。
- 离散 GPU -> VFIO PCI passthrough：提供最高性能，适合深度学习、3D 渲染、游戏等高负载任务（需要独占设备）。
- 不能/不想做硬件透传 -> virtio-gpu + virgl/Spice：最简单、兼容性好，但图形能力有限且不支持 CUDA 等通用计算。

### 3) 操作步骤示例
1) GVT-g（mdev）示例：
  ```bash
  # 启用 i915 gvt（需要 kernel cmdline: i915.enable_gvt=1）
  # 重启后：
  ls /sys/bus/pci/devices/0000:00:02.0/mdev_supported_types
  # 创建虚拟 GPU（会返回 uuid）
  echo <mdev-type> > /sys/bus/pci/devices/0000:00:02.0/mdev_supported_types/<mdev-type>/create
  # 假如输出 uuid=11111111-2222-3333-4444-0123456789ab
  # 将 uuid 绑定到虚拟机：
  virsh attach-device myvm /tmp/mdev_device.xml
  # 或 使用 vmdemo HTTP API: /vm/attach-mdev?name=myvm&uuid=11111111-2222-3333-4444-0123456789ab
  ```
  mdev XML 示例（libvirt）：
  ```xml
  <hostdev mode="subsystem" type="mdev" managed="yes">
    <source>
      <address uuid="11111111-2222-3333-4444-0123456789ab"/>
    </source>
  </hostdev>
  ```

2) VFIO PCI passthrough 示例（独占设备）：
  ```bash
  # 以 root 运行
  # unbind host driver
  echo 0000:01:00.0 > /sys/bus/pci/devices/0000:01:00.0/driver/unbind
  # bind to vfio-pci either by writing ids or via kernel cmdline vfio-pci.ids=xxxx:yyyy
  echo 8086 46d4 > /sys/bus/pci/drivers/vfio-pci/new_id
  # virsh xml 添加 hostdev pci
  virsh attach-device myvm /tmp/pci_device.xml
  ```

3) Virtio-gpu / SPICE：
  ```bash
  # 在 libvirt domain XML 中，使用 virtio-gpu 视频模型并启用 spice gl=on
  # virt-install 或 virsh edit 添加 video & graphics
  ```

### 4) vmdemo 后端集成建议
- 后端（`kvm/vm.go`）已新增：
  - `AttachPCIDevice(vmName, pciAddr)` & `DetachPCIDevice(vmName, pciAddr)`
  - `AttachMdevDevice(vmName, uuid)` & `DetachMdevDevice(vmName, uuid)`
- 对应 REST API（位于 service/vmops.go）：
  - `/vm/attach-pci?name=<vm>&pciAddr=0000:00:02.0`
  - `/vm/detach-pci?name=<vm>&pciAddr=0000:00:02.0`
  - `/vm/attach-mdev?name=<vm>&uuid=<uuid>`
  - `/vm/detach-mdev?name=<vm>&uuid=<uuid>`
- 权限说明：这些接口会调用 `virsh` 需要主机上安装 libvirt/qemu/kvm 并具有相应权限（通常为 root 或 libvirt 管理用户）。

### 5) 注意事项
- 在主机上进行 PCI 透传或 mdev 创建时，请确保宿主机没有使用该设备（或有其他后备方案），以免对宿主图形造成影响。
- 对于集成 GPU（iGPU），mdev 是最佳选择（如果支持），因为它可以在不占用整个 GPU 的情况下，给多个 VM 提供 GPU 资源。
- 对于需要高性能 GPU 的复杂工作负载（NVIDIA/AMD），优先选择离散 GPU VFIO 或厂商 vGPU（若有许可）。

以上内容可作为 vmdemo 后端的 GPU 虚拟化入门指南与实现说明。
